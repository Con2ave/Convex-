...
async def get_session_by_id(db: AsyncSession, session_id: int) -> Optional[StudySession]:
    """Retrieve a study session by its primary key, with its checks eager-loaded
    (response schemas may serialize .checks outside of an active session context)."""
    result = await db.execute(
        select(StudySession)
        .options(selectinload(StudySession.checks))
